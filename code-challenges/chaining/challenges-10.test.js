...
const count = (target, input) => {
  return input.reduce((accumulator, array) => {
    ...
    let isPresent = array.reduce((accumulatorSecond, value) =>{
      if(value === target){
        return accumulatorSecond + 1;
      }
...
const totalSum = (input) => {
  return input.reduce((accumulator, array) => accumulator += array.reduce((acculmulatorSecond, value) => acculmulatorSecond += value), 0);
};
...
let findShortest = (data) => {
  let shortestName = data.reduce((accumulator, value) => {
    if(accumulator.name.length >= value.name.length){
      accumulator = value;
    }
    return accumulator;
  }, data[0])
