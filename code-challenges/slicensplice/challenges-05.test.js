...
const howMuchPencil = (str) => {
  let listofwords = [];
  listofwords.push(str); //initial push
  const howlong = str //keep track of string length
  for(let i = 0; i < howlong.length; i++){
    str = str.slice(1)
    listofwords.push(str)
  }
  return listofwords;
};
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach(step => {
    let sentencearray = step.split(' ')
    // console.log(sentencearray);
    result.push(sentencearray[0]);
  })
  return result;
};
...
//for each index in arr, if % = 2, put in array/delete/
for(let i = 0; i < arr.length; i++) {
  if (arr[i] % 2 ===0) {
    arr.splice(i, 1) //splices out 2
    i--; 
  }
}
return arr;
