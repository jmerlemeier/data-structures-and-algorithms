...
const countNumberOfElements = (arr) => {
  let counted = arr.reduce((accumulator, value, index) => {
    accumulator =  index + 1;
  return accumulator;
  }, 1);
  return counted;
...
const returnNames = (arr) => {
  const characters = arr.reduce((accumulator, value, index) => {
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((accumultor, current) => {
    if(current.children){
      return current.children.length + accumultor;
    }
    return accumultor;
  }, 0);
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((accumultor, value) => {
    if(isPrime(value)){
      return accumultor + 1;
    }
    return accumultor;
